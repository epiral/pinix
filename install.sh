...
if [ "$INSTALL_MODE" = "cli" ]; then
  echo ""
  echo "================================================"
  echo "  Pinix CLI installed"
  echo "================================================"
  echo ""
  echo "  Pinix CLI connects to a remote Pinix Hub and"
  echo "  invokes Clips running on your other devices."
  echo "  No local daemon needed."
  echo ""
  echo "  Quick start:"
  echo "    pinix login                      browser-based login"
  echo "    pinix login --token pnx_...      token-based login"
  echo ""
  echo "  After login:"
  echo "    pinix registry search <query>    search Clips"
  echo "    pinix hub add @pinix/todo        install a Clip (on remote provider)"
  echo "    pinix hub list                   see available Clips"
  echo "    pinix invoke todo list           call a Clip"
  echo ""
  echo "  Note: Clips are served by Providers (other devices"
  echo "  running 'pinix start'). At least one Provider must"
  echo "  be online for invoke to work."
  echo ""
  echo "  Want the full runtime on this machine?"
...
echo "    pinix          Pinix CLI + daemon"
echo "    node / npm     Node.js runtime"
echo "    bun            Clip runtime (runs TypeScript Clips)"
echo "    bb-browser     Browser automation + live stream"
echo ""
echo "  What happens automatically on first run:"
echo "    Chrome          Downloaded on first 'pinix start'"
echo "    bb-viewer       Downloaded on first stream connection"
echo ""
echo "  Get started:"
echo ""
echo "    1. pinix start                 start the daemon"
echo ""
echo "    2. Log in (choose one):"
echo "       pinix login                 browser-based (opens a link to authorize)"
echo "       pinix login --token pnx_... token-based (get token from pinixai.com/settings)"
echo ""
echo "    3. open http://localhost:9000   open the Console"
echo ""
echo "  Useful commands:"
echo "    pinix hub list                 see installed Clips"
echo "    pinix hub add @pinix/todo      install a Clip"
echo "    pinix invoke todo list         use a Clip"
echo "    pinix stop                     stop the daemon"
echo ""
